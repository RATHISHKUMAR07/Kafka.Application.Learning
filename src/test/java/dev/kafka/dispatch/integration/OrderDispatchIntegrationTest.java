...
import dev.kafka.dispatch.message.DispatchPreparing;
...
import static dev.kafka.dispatch.integration.WiremockUtils.stubWiremock;
...
//                        container.getContainerProperties().getTopics().length * embeddedKafkaBroker.getPartitionsPerTopic()));
        registry.getListenerContainers().forEach(container ->
                ContainerTestUtils.waitForAssignment(
                        container,
                        embeddedKafkaBroker.getPartitionsPerTopic() * container.getContainerProperties().getTopics().length
                )
        );
